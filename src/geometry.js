...
function buildFaceWithConnections(builder, vol, axis, side, connections, selectedFace, flip = false) {
    ...
    buildWallWithMultipleHoles(builder, axis, innerPos, side, ru0, ru1, rv0, rv1, holes, wallFaceId, wallHl);
...
function buildWallWithMultipleHoles(builder, axis, pos, side, ru0, ru1, rv0, rv1, holes, faceId, hl) {
    const n = axis === 'x'
        ? (side === 'min' ? [1, 0, 0] : [-1, 0, 0])
        : (side === 'min' ? [0, 0, 1] : [0, 0, -1]);
...
function buildTunnelQuads(builder, axis, side, innerPos, outerPos, du0, du1, dv0, dv1, t, tunnelFaceId) {
    ...
    if (axis === 'x') {
        ...
        builder.addQuad(
            ...
            [0, 0, -1], [0, 0], [t, 0], [t, dh], [0, dh], tunnelFaceId, false
        );
        // Right side (at du1 = z)
        builder.addQuad(
        ...
        builder.addQuad(
            ...
            [0, -1, 0], [0, 0], [t, 0], [t, dw], [0, dw], tunnelFaceId, false
        );
        // Floor
        builder.addQuad(
        ...
        builder.addQuad(
            ...
            [-1, 0, 0], [0, 0], [t, 0], [t, dh], [0, dh], tunnelFaceId, false
        );
        // Right side (at du1 = x)
        builder.addQuad(
...
function buildExitCap(builder, axis, side, outerPos, du0, du1, dv0, dv1, faceId, hl) {
    ...
    if (axis === 'x') {
        builder.addQuad(
            ...
            nOut, [0, 0], [dw, 0], [dw, dh], [0, dh], faceId, hl
        );
    } else {
        builder.addQuad(
