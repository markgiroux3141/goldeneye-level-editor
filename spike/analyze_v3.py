...
def analyze(fpath, name):
    ...
    # Analysis 4: Use KDTree for efficient neighbor search
    print("\n[4] NEIGHBOR INFLUENCE & SPATIAL SMOOTHNESS")
    print("  Building KDTree for {} vertices...".format(len(colors)))
    
    color_list = list(colors.keys())
    coord_array = np.array([vertices[vi] for vi in color_list])
    tree = cKDTree(coord_array)
    
    print("  Querying neighbors within 100 units...")
    diffs = []
    pairs_found = 0
    
    for i, vi in enumerate(color_list[:min(5000, len(color_list))]):
        neighbors = tree.query_ball_point(coord_array[i], r=100)
        for j in neighbors:
            if j <= i: continue
            vj = color_list[j]
            bright_diff = abs(brightness(colors[vi]) - brightness(colors[vj]))
            d = coord_array[j] - coord_array[i]
            dist = math.sqrt(d[0]**2 + d[1]**2 + d[2]**2)
            diffs.append((dist, bright_diff))
            pairs_found += 1
            if pairs_found >= 50000: break
        if pairs_found >= 50000: break
    
    if diffs:
        diffs = np.array(diffs)
        corr = np.corrcoef(diffs[:, 0], diffs[:, 1])[0, 1] if len(diffs) > 1 else 0
        print("  Neighbor pairs found: {}".format(len(diffs)))
        ...
        else: print("    -> Weak/mixed correlation")
